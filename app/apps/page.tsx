...
import { filterAppScoped, getPagePrincipal, hasPermission } from "@/lib/iam";
...
export default async function AppsPage() {
  ...
  return <AppShell active="Aplicaciones" requiredPermission="apps.read"><PageHeader eyebrow="Fleet control" title="Aplicaciones" description={principal.allApps ? "Cada web tiene identidad, credenciales, políticas y tracking independientes aunque todas compartan mail.lvltechmx.com." : `Vista limitada a ${apps.length} aplicación${apps.length === 1 ? "" : "es"} autorizada${apps.length === 1 ? "" : "s"}.`} action={canManage ? <Link className="button" href="/apps/new">+ Agregar web</Link> : undefined} />
    {canManage && <section className="quick-onboarding-banner">
