...
export default async function AppTemplatesPage({ params }: { params: Promise<{ appId: string }> }) {
  ...
  return <AppShell active="Aplicaciones"><PageHeader eyebrow="Template Studio" title={`Plantillas · ${app.name}`} description="Diseña, previsualiza, prueba y versiona los correos de esta web sin editar HTML ni redeployar la aplicación." action={<Link className="secondary-button" href={`/apps/${appId}`}>← Control center</Link>} />
    <section className="platform-notice"><strong>Safe-by-design</strong><span>Los CTA críticos, OTP, URLs HTTPS, prioridad y tracking siguen controlados por LVL Mail aunque cambie el copy.</span></section>
