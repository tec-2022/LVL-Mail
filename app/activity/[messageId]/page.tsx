...
export default async function MessageTrackingPage({ params }: { params: Promise<{ messageId: string }> }) {
  ...
  return <AppShell active="Actividad"><PageHeader eyebrow="Search & Recovery" title={statusLabels[message.status] ?? message.status} description={`Diagnóstico y trazabilidad completa de un correo de ${app?.name ?? message.app_id}.`} action={<div className="header-actions"><Link className="secondary-button" href="/activity">← Buscar correos</Link><Link className="secondary-button" href={`/apps/${message.app_id}`}>Ver aplicación →</Link></div>} />
    ...
    <section className="panel recovery-panel"><div className="panel-head"><div><span className="eyebrow">SAFE REPLAY</span><h2>Recuperación controlada</h2></div><span className={replayDecision.allowed ? "pill success" : "pill neutral"}>{replayDecision.allowed ? "Disponible" : "Protegido"}</span></div>
      <RecoveryActions
        messageId={message.id}
        decision={replayDecision}
        recoveryExpiresAt={replayContext.recoveryExpiresAt}
        replayMessageId={replayContext.replayMessageId}
        secondaryReady={topology.secondaryReady}
        manualFailover={topology.failoverMode === "manual"}
      />
