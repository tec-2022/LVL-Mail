...
import { searchTrackedMessages } from "@/lib/search-recovery";
...
export default async function ActivityPage() {
  const [messages, apps] = await Promise.all([
    searchTrackedMessages({ limit: 100 }),
    listRegisteredApps(),
  ]);
  ...
  return <AppShell active="Actividad"><PageHeader eyebrow="Search & Recovery" title="Encontrar, diagnosticar y recuperar" description="Busca cualquier intento por tracking, aplicación, plantilla, estado, proveedor, fecha o destinatario exacto. LVL Mail mantiene la dirección fuera del ledger y solo habilita recuperación cuando no existe riesgo de duplicar o reutilizar credenciales." />
