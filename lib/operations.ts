...
export async function getFleetReliability(): Promise<FleetReliability[]> {
  if (!reliabilityConfigured()) return [];
  try {
    return await request<FleetReliability[]>("/rest/v1/rpc/mail_reliability_fleet", {
      method: "POST",
      body: "{}",
    });
  } catch {
...
export async function listIncidents(limit = 100): Promise<MailIncident[]> {
  if (!reliabilityConfigured()) return [];
  try {
    return await request<MailIncident[]>(
      `/rest/v1/mail_incidents?select=*&order=last_detected_at.desc&limit=${Math.min(Math.max(limit,1),200)}`,
    );
  } catch {
    return [];
  }
